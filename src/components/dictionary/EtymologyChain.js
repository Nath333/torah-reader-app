// =============================================================================
// Etymology Chain Component - PRO SCHOLAR v3
 ...
 */
const EtymologyChain = ({ root, rootInfo: providedRootInfo, compact = false }) => {
  const [expanded, setExpanded] = useState(false);

  // Get root info if not provided
  const rootInfo = useMemo(() => {
    return providedRootInfo || getRootInfo?.(root);
  }, [root, providedRootInfo]);

  // Build the etymology chain
  const chain = useMemo(() => {
