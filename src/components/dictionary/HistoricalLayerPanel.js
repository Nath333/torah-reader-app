 ...
 *
 * PRO SCHOLAR V18: Enhanced with dictionary-based fallback
...
// PRO SCHOLAR V18: Dictionary imports for fallback
...
// =============================================================================
// PRO SCHOLAR V18: DICTIONARY-BASED HISTORICAL LAYER EXTRACTION
// =============================================================================
...
// =============================================================================
// PRO SCHOLAR V19: SEMANTIC-BASED PERIOD INFERENCE
// =============================================================================
 ...
 * PRO SCHOLAR V19: Uses enriched dictionary semanticField data
 */
...
const loadDictionaryFallback = async (word, root) => {
  ...
  try {
    const [bdbEntry, jastrowEntry] = await Promise.all([
    ...
    // PRO SCHOLAR V19: Use semantic field for period inference when no explicit periods found
    ...
    // PRO SCHOLAR V19: Build characteristics from semantic field
    ...
    return {
      ...
      evolution: Object.keys(evolution).length > 0 ? evolution : null,
      isAramaic,
      semanticField,  // PRO SCHOLAR V19: Include for display
      briefMeaning,
      source: 'dictionary',
      hasData: true
    };
...
function HistoricalLayerPanel({
  ...
  // PRO SCHOLAR V18: State for dictionary fallback data
  ...
  const curatedAnalysis = useMemo(() => {
    ...
    // If the lookup word is long (derived form), try to extract the 3-letter root
    if (wordToLookup && wordToLookup.length > 4 && !HISTORICAL_EVOLUTION?.[wordToLookup]) {
      // Try action noun pattern: יציאות → יצא
      if (wordToLookup.endsWith('ות') && wordToLookup.length >= 5) {
        const stem = wordToLookup.slice(0, -2);
        if (stem.length === 4 && stem[2] === 'י') {
          const extractedRoot = stem[0] + stem[1] + stem[3];
          if (HISTORICAL_EVOLUTION?.[extractedRoot]) {
            wordToLookup = extractedRoot;
          }
        }
      }
      // Try feminine singular: יציאה → יצא
      else if (wordToLookup.endsWith('ה') && wordToLookup.length >= 4) {
        const stem = wordToLookup.slice(0, -1);
        if (stem.length === 4 && stem[2] === 'י') {
          const extractedRoot = stem[0] + stem[1] + stem[3];
          if (HISTORICAL_EVOLUTION?.[extractedRoot]) {
            wordToLookup = extractedRoot;
  ...
  // PRO SCHOLAR V18: Load dictionary fallback if curated data not available
  useEffect(() => {
    ...
    const lookupWord = root || word;
    if (!lookupWord) return;

    setIsLoadingFallback(true);

    loadDictionaryFallback(word, root)
      .then(data => {
  ...
  }, [word, root, curatedAnalysis?.hasData]);
  ...
  return (
    <div className={panelClassName}>
      ...
      {/* PRO SCHOLAR V19: Semantic field display */}
