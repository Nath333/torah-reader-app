 ...
 *
 * Enhanced with citation copy, uncertainty badges,
...
/**
 * Generate academic citation for a source
 */
...
/**
 * Get uncertainty level based on source agreement
 */
...
/**
 * Check if word is hapax legomenon (appears only once)
 */
 ...
 * Enhanced with hapax detection and citation copy
 */
const SourceCard = memo(function SourceCard({ source, isMain, word, onCopyCitation }) {
  ...
  const handleCopyCitation = useCallback(async (format) => {
    ...
    <div className={`source-card ${tierClass} ${isMain ? 'main-source' : ''} ${hapaxInfo.isHapax ? 'hapax' : ''}`}>
      ...
      {source.headword && (
        <div className="source-headword" dir="rtl" lang="he">
          {source.headword}
          {/* Show match type for morphological lookups */}
      ...
      {/* Wiktionary etymology data (Proto-Semitic, cognates) */}
      {source.etymology && (
 ...
 * Enhanced with uncertainty badges, citation export, and hapax detection
 */
const SourceComparison = memo(function SourceComparison({
  ...
  // Handle citation copy (hooks must be called before early returns)
  ...
  const handleCopyAllCitations = useCallback(async () => {
    if (!sources) return;
