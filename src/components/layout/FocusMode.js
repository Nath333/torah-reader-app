...
const FocusMode = React.memo(function FocusMode({
  ...
  const [copyFeedback, setCopyFeedback] = useState(null); // 'ref' | 'all' | null
  ...
  const hebrewWords = useMemo(() => {
    if (!currentVerse?.hebrewText) return [];
    return currentVerse.hebrewText.split(/\s+/).filter(w => w.length > 0);
  }, [currentVerse?.hebrewText]);

  // Gematria calculations for current verse
  const verseGematria = useMemo(() => {
    if (!currentVerse?.hebrewText) return { total: 0, words: [] };
    const words = getWordGematria(currentVerse.hebrewText);
    const total = calculateGematria(currentVerse.hebrewText);
    return { total, words };
  }, [currentVerse?.hebrewText]);
      ...
      <div className="focus-content" onClick={zenMode ? () => setZenMode(false) : undefined}>
        ...
        {studyMode === 'context' && contextVerses?.prev && (
          <div className="context-verse context-prev">
            ...
            <p className="context-hebrew" dir="rtl" lang="he">{contextVerses.prev.hebrewText}</p>
          </div>
        )}
        ...
        <div className={`verse-spotlight ${isTransitioning ? 'fade-out' : 'fade-in'} ${highlightedVerses.has(currentVerse?.verse) ? 'verse-highlighted' : ''}`}>
          ...
          {!zenMode && (
            <div className="scholar-quick-actions">
              <button
                className={`quick-action-btn ${copyFeedback === 'ref' ? 'copied' : ''}`}
                onClick={() => {
                  const ref = `${selectedBook} ${selectedChapter}:${currentVerse?.verse}`;
              ...
              <button
                ...
                onClick={() => {
                  const text = `${currentVerse?.hebrewText}\n\n${currentVerse?.englishText}\n\n— ${selectedBook} ${selectedChapter}:${currentVerse?.verse}`;
              ...
              >
                ...
                <span className="qa-label">גימטריא</span>
              </button>
              <button
                className={`quick-action-btn ${showMeforshimPanel ? 'active' : ''}`}
                onClick={() => setShowMeforshimPanel(!showMeforshimPanel)}
                title="Commentaries"
              >
                <span className="qa-icon">📚</span>
                <span className="qa-label">מפרשים</span>
              </button>
            </div>
          )}
          ...
          {studyMode === 'learning' ? (
            ...
            >
              {revealTranslation ? (
                <>
                  <p className="english-text-focus">{currentVerse?.englishText}</p>
            ...
            <>
              ...
              {showOnkelos && currentOnkelos && (
                <div className="onkelos-text-focus" dir="rtl" lang="arc">
                  <span className="onkelos-label">Targum:</span>
                  <ClickableText
                    language="aramaic"
                    text={currentOnkelos.aramaic}
        ...
        {studyMode === 'context' && contextVerses?.next && (
          <div className="context-verse context-next">
            ...
            <p className="context-hebrew" dir="rtl" lang="he">{contextVerses.next.hebrewText}</p>
          </div>
        )}
      </div>
