...
const TzuratHaDaf = ({
  ...
  useEffect(() => {
    // If no verses, set loading to false immediately
    if (!verses || verses.length === 0) {
      setLoading(true); // Keep loading while waiting for verses
    ...
    const loadCommentaries = async () => {
      ...
      try {
        // Load commentaries in parallel with timeout protection
        const timeoutMs = 10000; // 10 second timeout per request
        ...
        const [rashiResult, tosafot, maharsha] = await Promise.all([
          withTimeout(getRashiOnTalmud(selectedBook, selectedChapter), timeoutMs)
            .catch(err => {
              console.error('[TzuratHaDaf] Rashi load error:', err);
              return { comments: [] };
            }),
          withTimeout(getTosafotForDaf(selectedBook, selectedChapter), timeoutMs)
            .catch(err => {
              console.error('[TzuratHaDaf] Tosafot load error:', err);
              return [];
            }),
          withTimeout(getMaharshaForDaf(selectedBook, selectedChapter), timeoutMs)
            .catch(err => {
              console.error('[TzuratHaDaf] Maharsha load error:', err);
              return { halachot: [], aggadot: [] };
            })
        ]);
        ...
        // Still show content even if commentaries fail
        setLoadingProgress(100);
      }
      ...
      <div className="tzurat-hadaf loading">
        <div className="daf-loading-spinner">
          ...
          <div className="daf-loading-text">
            <span className="daf-loading-hebrew" dir="rtl">{loadingMsg.hebrew}</span>
