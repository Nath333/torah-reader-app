...
// Track which proxy is working + automatic recovery
let activeProxyIndex = 0;
let proxyFailureTimestamp = 0;
const PROXY_RESET_INTERVAL = 5 * 60 * 1000; // Reset to primary proxy after 5 minutes

/** Track last error for diagnostics */
let lastError = null;

/**
 * Get active proxy index, resetting to primary if enough time has passed
 * since the last failure (allows recovery from transient errors)
 */
const getActiveProxyIndex = () => {
  if (activeProxyIndex > 0 && Date.now() - proxyFailureTimestamp > PROXY_RESET_INTERVAL) {
    activeProxyIndex = 0; // Give primary proxy another chance
  }
  return activeProxyIndex;
};

/**
 * Get the last CAL API error (for diagnostics/UI)
 * @returns {Object|null} Last error info
 */
 ...
 */
const buildCalUrl = (endpoint, proxyIndex = getActiveProxyIndex()) => {
  if (IS_DEV) {
 ...
 */
const getCurrentProxy = () => CORS_PROXIES[getActiveProxyIndex()] || CORS_PROXIES[0];
 ...
 */
const fetchCalData = async (endpoint, proxyIndex = getActiveProxyIndex()) => {
  ...
  try {
    ...
    if (!IS_DEV && nextProxyIndex < CORS_PROXIES.length) {
      // Update active proxy for future requests and track when failure occurred
      activeProxyIndex = nextProxyIndex;
      proxyFailureTimestamp = Date.now();
      return fetchCalData(endpoint, nextProxyIndex);
    }

    // All proxies exhausted — record error for diagnostics
    lastError = {
      message: error.message,
      endpoint,
      proxy: proxy.name,
      timestamp: Date.now(),
      allProxiesExhausted: true
    };

    if (IS_DEV) {
      console.warn('CAL fetch error (all proxies exhausted):', error.message);
    }
    return null;
  }
};

// Cache for CAL lookups with unified telemetry
