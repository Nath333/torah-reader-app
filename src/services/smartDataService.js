...
export const smartLookup = async (word, options = {}) => {
  ...
  try {
    const { scholarlyLookup } = await import('./scholarlyLexiconService');
    ...
    if (result?.primaryDefinition) {
      ...
      if (includeFrench && output.english) {
        try {
          const { translateEnglishToFrench } = await import('./englishToFrenchService');
      ...
      storeOffline(STORES.LOOKUP, { word: cleaned, ...output });
...
export const smartRAG = async ({
  ...
  try {
    ...
    if (context?.sources?.length > 0) {
      ...
      storeOffline(STORES.RAG, { key: cacheKey, reference: `${book}.${chapter}.${verse}`, ...context });
